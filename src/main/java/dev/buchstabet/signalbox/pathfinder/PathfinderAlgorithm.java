...
public class PathfinderAlgorithm {
    ...
    public CompletableFuture<Collection<PathPosition>> paintBestWay() {
        return CompletableFuture.supplyAsync(() -> {
            ...
            while (foundTarget.getFrom() != null) {
                ...
                byte toSet = fromToCheck(fromData.getPosition(), throughData.getPosition(), toData.getPosition());


                throughData.set(toSet);
                if (throughData instanceof SignalPositionData) {
                    SignalPositionData signalPositionData = (SignalPositionData) throughData;

                    if (signalPositionData.getSignal() != SignalPosition.BEACON)
                        signalPositionData.setSignalPosition(SignalPosition.DRIVE);
                }
            }
            if (frt) {
                SignalGui.getInstance().getFfRT().use();
            }
            start.getPositionData().ifPresent(data -> {
                if (((SignalPositionData) data).getSignal() != SignalPosition.BEACON) {
                    ((SignalPositionData) data).setSignalPosition(SignalPosition.DRIVE);
                }
            });
            ...
            if (positions.size() > 1) {
                positions.get(positions.size() - 1).getPositionData().ifPresent(data -> positions.get(positions.size() - 2).getPositionData().ifPresent(data1 -> {
                    if (data instanceof SignalPositionData) {
                        ((SignalPositionData) data).setNext(data1.getLocation());
                    }
                }));
            }
    ...
    public static byte fromToCheck(Position from, Position through, Position to) {
